...
public static class MemoryXmlParser
{
    public static IXmlNode Parse(string xmlContent)
    {
        var xmlContentAsSpan = xmlContent.AsMemory();
        var result = TryParseNode(xmlContentAsSpan, startIndex: 0);
        if (!result.IsSuccess)
        {
    ...
    private static IParseResult<IXmlNode> TryParseNode(ReadOnlyMemory<char> xmlContent, int startIndex)
    {
    ...
    private static ParseResult<IXmlNode> TryParseTextNode(ReadOnlyMemory<char> xmlContent, int startIndex)
    {
        ...
        return ParseResult<IXmlNode>.Success(new XmlTextNode(textContent), afterTextIndex);
    }

    private static IParseResult<XmlTag> TryParseXmlTag(ReadOnlyMemory<char> xmlContent, int startIndex)
    {
        ...
        return ParseResult<XmlTag>.Success(new XmlTag(nodeName, attributes.Value, childNodes), currentIndex);
    }

    private static ParseResult<List<XmlAttribute>> ParseAttributes(ReadOnlyMemory<char> xmlContent, int startIndex)
    {
    ...
    private static ParseResult<XmlAttribute> TryParseAttribute(ReadOnlyMemory<char> xmlContent, int currentIndex)
    {
        if (xmlContent.LookAheadFor(currentIndex, lookFor: '>'))
        {
        ...
        return ParseResult<XmlAttribute>.Success(new XmlAttribute(attributeName, attributeValue), currentIndex);
    }
}
